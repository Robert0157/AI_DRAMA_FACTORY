...
**本版重點（v16.2.13）**：
- **首幀素材池正式定義（§4.8）**：A 線首幀一律取自 `first_frames/`（系統生成）＋`VEO_download/`（CEO 手動匯入）；索引 `manifest.json`（逐檔 sha256／尺寸／命名）；**水印複核**——全池 166 張底部帶雙向掃描，2 檔帶標記（Meta AI／小紅書 ID）隔離 `_水印隔離/`（僅得作草稿），164 張初篩可用。
...
| 水印複核（2026-09-13） | 全池 166 張底部帶雙向掃描：**2 檔帶標記**（Meta AI／小紅書 ID）已隔離至 `VEO_download/_水印隔離/`（僅得作草稿；**嚴禁去除／上架**）；記錄 `CEO/02_素材與CP-D/水印複核_20260913/複核結果.md` |
...
| 首幀池水印複核 | 全池 166 張底部帶雙向掃描＋局部放大複驗 | **2 檔隔離、164 張初篩可用**（2026-09-13） |
...
## 13. 變更歷史
- **v16.2.13（2026-09-13）**：**首幀素材池定義＋全池水印複核＋CEO 控台 UI 提案**——① 首幀池＝`first_frames`（系統生成）＋`VEO_download`（CEO 手動）寫入 §4.8；manifest 逐檔指紋② 全池 166 張水印複核：2 檔帶標記（Meta AI／小紅書 ID）隔離 `_水印隔離/`、164 張初篩可用（記錄入 CEO 樹）③ 交付《CEO控台UI完善提案_v1》：五入口盤點＋Streamlit 雙線控台三階段（P1 唯讀總覽→P2 一鍵決定→P3 深度），待 CEO 圈選。
