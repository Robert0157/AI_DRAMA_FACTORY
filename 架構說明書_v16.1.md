...
**本版重點（v16.2.18）**：
- **Pexels 停用與 RCA（CEO 2026-09-17 指示）**：① 每日素材搜尋排程**取消**（Mac launchd 已 unload＋disable）② RCA 結論：**金鑰有效**——403「error code: 1010」＝ Cloudflare 封鎖非瀏覽器 UA（`key+browserUA` 實測 200）；`stock_search.py` 全面改瀏覽器 UA；③ 診斷工具 `rca_pexels.py`（指紋化，不列印金鑰）。
**本版重點（v16.2.17）**：
- **分發與市場驗證第一階段（§4.9）**：分發帳冊（`distribution_ledger.py`，合併 Short＋Long 上傳史）＋YouTube 公開統計回收（`yt_metrics_collector.py`→`market_feedback.db`）＋週報評級 scale／iterate／retire（`market_validation_report.py`→`CEO/05_市場驗證/`）＋節目包契約（`episode-short-v1`；175s 影片原生 Shorts）＋Mac 上傳器與排程（`market.metrics` 每日 03:30、`market.report` 週日 09:30，已安裝）；**實證發現：Shorts 累計 220 支（light_music 152／lofi 68，最後 2026-09-09）後佇列已空＝供給斷鏈（非上傳障礙）**。
**本版重點（v16.2.16）**：
- **CEO 隨時增刪韌性（CEO 2026-09-14 指示）**：`inbox/`、`VEO_download/`（含 `Approved_material/`）、`cp_d/` 內容可隨時上傳／刪除——`daily_intake.py`／`stock_search.py`／`pool_manifest.py` 全面容錯（列舉／stat／寫入逐項防護、資料夾被刪自動重建、檔案半途消失＝略過）；清除判準改 **max(mtime, 出生時間, ctime)**（防舊時間戳拷貝被誤刪）；manifest 被刪／半寫＝視為空重建；池重建含 `skipped` 統計。
**本版重點（v16.2.15）**：
- **素材 intake 改版（CEO 2026-09-14 三指示）**：**固定投遞夾** `assets/reference_intake/inbox/`（廢除週子夾）＋**每日 02:00** 搜尋＋**隔日清除**（未移出者次日 02:00 刪）＋**Approved_material 審核流**（CEO 以實體移動批准→首幀池第三來源）＋**跨日去重註冊表** `_state/seen_assets.json`；`weekly_intake.py`→`daily_intake.py` 退役；世界草案引擎暫停待改版。
- **步數盲測結案（CEO 2026-09-14）**：20／16／12／10 步四支**皆不滿意**→步數槓桿**暫停**，渲染替代方案待 CEO 指示；測試包＋結果備註入檔（`CEO/02_素材與CP-D/步數盲測_20260914/`）。
**本版重點（v16.2.14b）**：
- **VEO 生圖提示詞定版（Cinemagraph 式）＋首輪十二鏡重發（CEO 2026-09-13 實證）**：固定機位・零運鏡・微量動態清單・其餘凍結・`Seamless 8-second loop`；禁含運鏡動詞與製作用語（舊式產圖平庸）。§4.3 增列格式規格；提示詞檔同名重發。
**本版重點（v16.2.14a）**：
- **A 線首輪選定（CEO 2026-09-13）**：**lofi＝華服荒原 v5「章回編年」**；**light_music＝湖光微火 v2「記憶檔案」**；生圖派工包交付（`CEO/02_素材與CP-D/A線首輪審核_20260913/`，落點 `VEO_download/生圖_*`）。下一步＝Flow 免費生圖 → 首幀圈選 → Mac 渲染。
**本版重點（v16.2.14）**：
- **A 線可驗證性實證＋第一 Checkpoint 交付（2026-09-13）**：PC 一鍵產線 `run_mv_pipeline.py` 全鏈重跑——`status`（Toonflow 3 專案可達／ComfyUI 待命／handoff done 5）→ `scripts`（3×5 世界優先合約 **15/15 通過**）→ `novel --llm`（15 章小說體審核稿）→ `review`（重新匯入 Mac Toonflow）。**第一個 CEO 檢查點（Toonflow 審核）已就位**：`CEO/02_素材與CP-D/A線首輪審核_20260913/`（清單＋三題材×五版）。
- **GATE 0 結案（CEO 判定）**：不需測試——VEO **影片**匯出 100% 帶浮水印（僅草稿）；**圖片免費且無水印**（首幀池即以此建立）；英雄鏡引擎改道評估待排。
- **首幀池命名正規化**：28 檔完成（對照表 `命名正規化對照表_20260913.md`）；**UI 方向核准**（優先級一般、不急）。
**本版重點（v16.2.13）**：
- **首幀素材池正式定義（§4.8）**：A 線首幀一律取自 `first_frames/`（系統生成）＋`VEO_download/`（CEO 手動匯入）；索引 `manifest.json`（逐檔 sha256／尺寸／命名）；**水印複核**——全池 166 張底部帶雙向掃描，2 檔帶標記（Meta AI／小紅書 ID）**已刪除**（CEO 同日指示），164 張初篩可用。
- **CEO 控台 UI 完善提案 v1 交付**：`CEO/04_架構檢視/CEO控台UI完善提案_v1.md`——現況五入口盤點、「雙線 CEO 控台（Streamlit）」三階段路線（P1 唯讀總覽→P2 一鍵決定→P3 深度）＋信箱式決策（`CEO_ACTIONS/`），待 CEO 圈選。
**本版重點（v16.2.12）**：
- **CEO 四項決策落地（2026-09-13）**：① forge 閘門採 **B＋ 先行弧**——`pilot_arc.json`（`forge.pilot_arc.v1`：Ep1–4 弧評分 ≥8.8／單域 ≥8.0／零 error＋bible/canon 完整＋迭代 ≥15）→ `--gate-check --pilot` → `lock_ready_pilot.json`（`forge.lock_pilot.v1`）＝**CP-D 入場證（先行弧）**；**量產仍須全包 `lock_ready.json`**。防偽：指紋綁定王者包（審後變更即 stale）／`run_meta` 對帳／離線證據不得授權；forge 離線測試 55 項（＋17）② 盲評 2（療癒慢鏡）：fixed **12** ＞ mir_v1 10 ＞ ambient 9——mir_v1 優勢未複現 → 預設維持、P3 opt-in 不變；樣式選擇改以素材形態為判據（v4 §10）③ GATE 0（VEO 水印預檢）操作卡＋入口 `CEO/02_素材與CP-D/VEO_download/`（保守取鏡；水印紅線 QC 不變）④ ②-2 測試保守 vs ⑥ lofi 延伸並行入檔。
- **雙線驗證**：**106 passed**（`ENGINEERING_QA=PASS PRODUCTION_RELEASE=BLOCKED`；CI 下限同步升至 106）。
**本版重點（v16.2.11）**：
...
> **一鍵產線（實證 2026-09-13）**：`python scripts/pipeline/run_mv_pipeline.py {status|scripts|novel|review|render}`——PC 控制面單一指令；Mac 端只留 Toonflow 審核與 Wan 渲染。全鏈實證：合約 15/15 通過、審核稿 15 章、Toonflow 3 專案×5 腳本就位（§4.2）。
...
2. CEO 在 Flow 免費生圖（**圖片免費且無水印**；CEO 2026-09-13 實證）→ 匯出放 `CEO/02_素材與CP-D/VEO_download/生圖_<題材>v<N>/`（shXXX.png）。
3. **CEO 批准畫面後才允許動用影片點數**（VEO／Kling）。
4. **提示詞格式（CEO 2026-09-13 實證定版）＝Cinemagraph 式**：① `Cinemagraph style`＋氛圍開場 ② 場景元素（世界錨點／色調／天候）③ `Fixed camera angle. Zero camera movement.` ④ 編號列舉微量動態（1)2)3)）⑤ 其餘凍結＋`Do NOT animate …` ⑥ 人物／文字約束（零對白・`no text`・`no logo`）⑦ `Seamless 8-second loop`。**禁含**運鏡動詞（push-in／crane／tracking）與製作用語（如「以題材鉤子畫面開場」）。
...
> **VEO 影片路線（CEO 2026-09-13 判定）**：VEO／Flow **影片**匯出 100% 帶可見浮水印 → **不得上架、僅作草稿**；英雄鏡引擎由 **Kling／付費 API／本地** 遞補（逐案核准）。**圖片**（Flow 免費生圖）無浮水印 → 首幀與草稿圖照常使用（§4.8 首幀池）。
...
| 規則 | 內容 |
|---|---|
| 頻率 | **每日 1 次（2026-09-14 改版）**；每次入庫 ≤ 3 座世界（防題材疲乏）；**排程於 2026-09-17 經 CEO 指示停用** |
...
| 落點鐵律 | **素材一律「固定投遞夾」`assets/reference_intake/inbox/`（無週子夾；2026-09-14 CEO 指示）**；審核包 `assets/reference_intake/cp_d/`（**暫停產出**）；禁止另立投遞資料夾（2026-09-12 CEO 糾正落實） |

**實作與排程（2026-09-14 改版：固定夾＋每日＋隔日清除）**

> **2026-09-14 改版（CEO 指示）**：① 投遞夾固定為 `assets/reference_intake/inbox/`（廢除 `2026-Wxx/` 週子夾）② 搜尋改**每日 02:00** ③ CEO 每日檢視 inbox，**把要用的圖移至 `CEO/02_素材與CP-D/VEO_download/Approved_material/`**（批准＝實體移動，最直覺）④ **未移出者次日 02:00 自動刪除**（18h 清除視窗；先清後抓）⑤ 跨日去重註冊表 `_state/seen_assets.json`（provider➕id；清 inbox 不影響）⑥ 世界草案引擎**暫停**（待改版為以 Approved_material 為輸入）⑦ **CEO 隨時增刪韌性**：三資料夾（`inbox`、`VEO_download`↔`Approved_material`、`cp_d`）內容可任意上傳／刪除——程式全面容錯；詳 `scripts/reference_intake/README.md`⑧ **排程停用（2026-09-17）**：Pexels 403/1010＝Cloudflare 封非瀏覽器 UA（金鑰有效、已修 UA）；每日搜尋排程經 CEO 指示取消。
| 構面 | 內容 |
|---|---|
...
| 排程入口 | **（已停用 2026-09-17）** `scripts/reference_intake/daily_intake.py`（先清過期素材→跑 `stock_search.py`→Telegram 摘要；冪等：同日已入庫即跳過，`--force` 可重跑） |
| Mac 排程 | **已取消（2026-09-17 unload＋disable）**；原 launchd `com.aidramafactory.reference.intake`＝每日 02:00（plist 版控於 `scripts/reference_intake/`） |
...
| 首航實績 | **2026-W37：20 張、failures=0（Pexels）**；**2026-09-14 遷移**：CEO 複篩 16 幀→`Approved_material/`（另附來源註記＋manifest 備份），當日批次 44 幀併入固定夾 inbox（6 幀位元級重複已去除；seen 註冊表種子 54 筆） |
...
| 每日流程整合 | `daily_intake.py`：清除→搜尋→Telegram 一步到位；**草案引擎暫停接線**（`--with-proposal` 保留旗標；改版後將以 `Approved_material/` 為輸入） |
| 下一步 | **Approve→commit 審核流**（解析 CEO 回覆→`world_library.commit()`；目前由人工指令執行） |

### 4.8 首幀素材池（CEO 2026-09-13 指示）

| 項 | 內容 |
|---|---|
| 定義 | A 線首幀圖形**一律取自**三處：`CEO/02_素材與CP-D/first_frames/`（系統生成；Flux GGUF）＋`CEO/02_素材與CP-D/VEO_download/`（**CEO 手動下載匯入**）＋`CEO/02_素材與CP-D/VEO_download/Approved_material/`（**每日素材經 CEO 移動批准者**；2026-09-14 指示） |
| 索引 | `CEO/02_素材與CP-D/manifest.json`（schema `ceo.first_frame_pool.v1`）：逐檔 sha256／尺寸／格式／命名檢查；圖片本體依 `.gitignore` 不入版控（媒體走 CEO 樹）；重建工具 `scripts/reference_intake/pool_manifest.py`（三來源彙整） |
| 水印複核（2026-09-13） | 全池 166 張底部帶雙向掃描：**2 檔帶標記**（Meta AI／小紅書 ID）**已刪除**（CEO 2026-09-13 指示）；往後帶標記素材一律不得入池（**嚴禁去除浮水印**）；記錄 `CEO/02_素材與CP-D/水印複核_20260913/複核結果.md` |
| 使用規則 | 入 I2V 前仍須過浮水印 QC（§4.6）；命名違規檔（空格／複製後綴）待 CEO 決定是否正規化 |

### 4.9 分發與市場驗證（v16.2.17，CEO 2026-09-14 指示）

> 目標：**內容快速分發＋市場驗證**閉環。上傳執行端仍在 Mac（§10a 授權）；本節＝控制面可見性＋數據回收＋決策規則。

| 項 | 內容 |
|---|---|
| 分發帳冊 | `scripts/marketing/distribution_ledger.py`——合併 `Shorts_Queue/*/upload_history.jsonl`＋`Long_Queue/*/long_upload_history.jsonl` → `assets/data/distribution_ledger.jsonl`；`--check`＝節奏報告（實際 vs 預期、staleness） |
| 數據回收 | `scripts/marketing/yt_metrics_collector.py`——YouTube Data API v3 公開統計（views／likes／comments）→ `assets/data/market_feedback.db`（快照制）；金鑰 Mac `market_validation.env`（`YT_DATA_API_KEY`；未設＝安全跳過、不報錯） |
| 週報評級 | `scripts/marketing/market_validation_report.py`——節奏＋成長 Top10＋**scale／iterate／retire** → `CEO/05_市場驗證/<ISO週>/報告.md`＋`metrics.json`（`--notify`＝Telegram 摘要） |
| 決策規則 | `configs/market_validation_policy.json`（初始門檻：Shorts scale≥800 觀看/週＆讚率≥3%、iterate≥200；可調不需改碼） |
| 節目包契約 | `scripts/marketing/episode_bundle.py`——175s 影片原生 Shorts 打包（schema `episode-short-v1`；9:16・≤185s・fail-closed）→ `Y:/Shorts_Queue/<頻道>/episodes/`；Mac `publish_episode_shorts.sh`（FIFO 上傳＋審計回寫同一 history） |
| Mac 排程 | `com.aidramafactory.market.metrics`（每日 03:30）＋`com.aidramafactory.market.report`（週日 09:30＋Telegram）；裝法 `install_market_validation_launchd.sh`（已安裝） |
| 實證發現（2026-09-14） | Shorts 累計 220 支成功（light_music 152／lofi 68；最後 2026-09-09）；**佇列已空＝供給斷鏈、非上傳障礙**；長片線自 2026-05-12 停擺；分析全文 `CEO/04_架構檢視/分發與市場驗證_缺口分析與第一階段補齊_v1.md` |
| Phase 2 | retention／CTR 需 YouTube Analytics API（OAuth `yt-analytics.readonly`）；B 線頻道上線後於 policy 啟用 cadence |
...
| 門檻（閘門） | **迭代 ≥15** ∧ **總分 ≥8.8** ∧ **單域 ≥8.0** ∧ **零 error**（全包）→ 寫 `lock_ready.json`（量產前置）。**B＋ 先行弧（v16.2.12）**：`--gate-check --pilot` 驗 `pilot_arc.json`（Ep1–4；同門檻＋bible/canon 完整；指紋綁定王者包、`run_meta` 對帳、離線不得授權）→ `lock_ready_pilot.json`＝**CP-D 入場證（先行弧）** |
...
| 工具 | `scripts/story_room/script_forge.py`（`--status`／`--gate-check`〔＋`--pilot` 先行弧〕／`--resume`／`--reclaim`／`--seed-package`；forge 離線測試 55 項〔含 B＋ 先行弧 17 項〕＋`llm_text` 退避 6 項）；`scripts/story_room/script_novel.py`（小說體審核稿） |
...
flowchart LR
  ...
  FG{P1.5 鑄造：迭代 ≥15＋好萊塢門檻（log 為證；B＋ 先行弧可先解鎖 CP-D）}
  FG -->|未達標持續迭代| FG
  FG -->|達標＝lock_ready（pilot／全包）| P2[P2 定裝：角色四視圖＋關鍵首幀＋VEO 備援提示詞]
...
| **P1.5 鑄造（v16.1.6）** | PC 控制面（DeepSeek 文字通道） | 迭代版企劃包＋逐迭代 log（`Auto_Drama/output/script_forge/<series>/iteration_log.md`） | **L1→L2→L3→L4 迭代 ≥15＋好萊塢門檻（總分 ≥8.8／單域 ≥8.0／零 error）；`lock_ready_pilot.json`（B＋ Ep1–4 先行弧）或 `lock_ready.json`（全包）＝CP-D 入場證；量產前須全包鎖** |
...
| VEO 浮水印 | 抽幀目視（新舊匯出檔） | **可見「Veo」浮水印**（2026-09-12；紅線：嚴禁去除、帶水印素材不得上架） |
| VEO 無浮水印路徑（GATE 0） | CEO 實證判定（2026-09-13） | **結案（不執行）**：VEO 影片匯出 100% 帶水印（僅草稿）；圖片免費無水印 |
| 首幀池水印複核 | 全池 166 張底部帶雙向掃描＋局部放大複驗 | **2 檔刪除、164 張初篩可用**（2026-09-13） |
| B 線 B＋ 先行弧閘門 | `pytest tests/test_script_forge.py`（＋17 項）＋雙線 CI | **PASS**；雙線 **106 passed**（2026-09-13） |
...
| 4 | A 線世界擴充基建：`reference_intake`＋免費圖庫 API 自動搜尋（`stock_search.py`）＋launchd **每日排程**＋CP-D Telegram＋**CEO 移動審核流** | 程式 | **✅ 每日 02:00＋固定夾＋隔日清除＋去重註冊表＋Approved_material 審核流已上線（2026-09-14 改版）；首航 20 張＋6 型制與選取回合 24 幀（2026-W37）**；世界視覺分析（propose_batch 接線）暫停待改版 |
| 5 | VEO 無浮水印路徑驗證 | 驗證 | **結案（CEO 2026-09-13）**：不需測試——VEO 影片匯出 100% 帶浮水印（僅得草稿）；**圖片免費且無水印**（首幀池即以此建立）。英雄鏡引擎改道評估（Kling／付費 API／本地）待排 |
...
| 10 | 雙線 CEO 控台（UI 完善提案：唯讀總覽／素材池／待決中心） | 程式 | **方向已核准（CEO 2026-09-13）**；**優先級一般、不急**——未排程 |

---

## 13. 變更歷史
- **v16.2.18（2026-09-17）**：**Pexels 停用與 RCA**——每日素材搜尋排程取消（unload＋disable）；RCA：金鑰有效、403/1010＝Cloudflare 封非瀏覽器 UA（browserUA 200）→`stock_search.py` 改瀏覽器 UA；`rca_pexels.py` 上線；§4.7 標註停用。
- **v16.2.17（2026-09-14）**：**分發與市場驗證第一階段**——分發帳冊＋YouTube 數據回收（Data API）＋週報評級（scale/iterate/retire）＋節目包契約（`episode-short-v1`）＋Mac 上傳器與排程（market.metrics／market.report，已安裝）；§4.9 新增；實證：Shorts 220 支上傳史、佇列已空（供給斷鏈、非上傳障礙）；測試 106→113。
- **v16.2.16（2026-09-14）**：**CEO 隨時增刪韌性**——三資料夾內容可任意上傳／刪除；`daily_intake`（清除判準 max(mtime, 出生時間, ctime)＋空夾回收＋通知改以現存檔計數）／`stock_search`（manifest 容錯＋資料夾自動重建）／`pool_manifest`（掃描逐項容錯＋skipped 統計＋root 缺失明確報錯）；README 增韌性章節。
- **v16.2.15（2026-09-14）**：**素材 intake 改版＋步數盲測結案**——固定投遞夾＋每日 02:00＋隔日清除＋Approved_material 審核流＋去重註冊表；`daily_intake.py` 上線（launchd 同日改版）、`weekly_intake.py` 退役；遷移完成（W37 16 幀→Approved_material／W38 44 幀入固定夾、去重 6、seen 種子 54）；§4.7／§4.8／§12-4 同步；首幀池重建（三來源）；步數四支 CEO 皆不滿意→槓桿暫停。
- **v16.2.14b（2026-09-13）**：**VEO 生圖提示詞定版（Cinemagraph 式）＋首輪十二鏡重發**——依 CEO 實證（固定機位・零運鏡・微量動態・其餘凍結・8 秒循環），改版 `生圖提示詞_lofi_華服荒原_v5.txt`／`生圖提示詞_light_music_湖光微火_v2.txt`；§4.3 增列格式規格與落點更新（`VEO_download/生圖_*`）。
- **v16.2.14a（2026-09-13）**：**A 線首輪選定＋生圖派工**——lofi＝華服荒原 v5／light_music＝湖光微火 v2（各 6 鏡提示詞包）；落點 `VEO_download/生圖_華服荒原v5`／`生圖_湖光微火v2`；下一步 Flow 免費生圖→首幀圈選→Mac 渲染（派送指令備妥未執行；建議先第一鏡樣片）。
- **v16.2.14（2026-09-13）**：**A 線可驗證性實證＋首輪 Checkpoint＋GATE 0 結案＋素材池正規化**——① `run_mv_pipeline.py` 全鏈：status／scripts（15/15 合約）／novel --llm（15 章）／review（Toonflow 3×5 就位）；審核包 `CEO/02_素材與CP-D/A線首輪審核_20260913/`② GATE 0 不執行（CEO：VEO 影片 100% 水印、圖片免費無水印）；VEO 路線註記 §4.4③ 首幀池 28 檔命名正規化（對照表入 CEO 樹）④ UI 提案方向核准（優先級一般、不急）。
- **v16.2.13（2026-09-13）**：**首幀素材池定義＋全池水印複核＋CEO 控台 UI 提案**——① 首幀池＝`first_frames`（系統生成）＋`VEO_download`（CEO 手動）寫入 §4.8；manifest 逐檔指紋② 全池 166 張水印複核：2 檔帶標記（Meta AI／小紅書 ID）**同日經 CEO 指示硬刪除**、164 張初篩可用（記錄入 CEO 樹）③ 交付《CEO控台UI完善提案_v1》：五入口盤點＋Streamlit 雙線控台三階段（P1 唯讀總覽→P2 一鍵決定→P3 深度），待 CEO 圈選。
- **v16.2.12（2026-09-13）**：**CEO 四項決策落地**——① forge 閘門採 **B＋ 先行弧**：`pilot_arc.json`（`forge.pilot_arc.v1`）→ `--gate-check --pilot` → `lock_ready_pilot.json`（`forge.lock_pilot.v1`）＝Ep1–4 解鎖 CP-D；量產仍須全包 `lock_ready.json`；指紋綁定王者包／`run_meta` 對帳／離線不得授權；forge 55 項（＋17）② 盲評 2（療癒慢鏡）：fixed 12 ＞ mir_v1 10 ＞ ambient 9——mir_v1 未複現 → 預設維持＋P3 opt-in 不變；樣式選擇改以素材形態為判據（v4 §10、設計草案 §10）③ GATE 0 操作卡＋`VEO_download/` 入口（保守取鏡：世界漫遊／手部／匿名女性側背；8s・9:16；匯出即無可見浮水印方可入線）④ ②-2 測試保守 vs ⑥ lofi 半身＋臉部特寫（已核准延伸）並行入檔。雙線驗證 **106 passed**（`ENGINEERING_QA=PASS`）。
