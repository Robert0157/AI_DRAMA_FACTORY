...
| 項 | 內容 |
|---|---|
| 定義 | A 線首幀圖形**一律取自**兩處：`CEO/02_素材與CP-D/first_frames/`（系統生成；Flux GGUF）＋`CEO/02_素材與CP-D/VEO_download/`（**CEO 手動下載匯入**） |
| 索引 | `CEO/02_素材與CP-D/manifest.json`（schema `ceo.first_frame_pool.v1`）：逐檔 sha256／尺寸／格式／命名檢查；圖片本體依 `.gitignore` 不入版控（媒體走 CEO 樹） |
