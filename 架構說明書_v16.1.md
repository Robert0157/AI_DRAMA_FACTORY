...
| 規則 | 內容 |
|---|---|
| 頻率 | **每日 1 次（2026-09-14 改版）**；每次入庫 ≤ 3 座世界（防題材疲乏） |
...
> **2026-09-14 改版（CEO 指示）**：① 投遞夾固定為 `assets/reference_intake/inbox/`（廢除 `2026-Wxx/` 週子夾）② 搜尋改**每日 02:00** ③ CEO 每日檢視 inbox，**把要用的圖移至 `CEO/02_素材與CP-D/VEO_download/Approved_material/`**（批准＝實體移動，最直覺）④ **未移出者次日 02:00 自動刪除**（18h 清除視窗；先清後抓）⑤ 跨日去重註冊表 `_state/seen_assets.json`（provider➕id；清 inbox 不影響）⑥ 世界草案引擎**暫停**（待改版為以 Approved_material 為輸入）⑦ **CEO 隨時增刪韌性**：三資料夾（`inbox`、`VEO_download`↔`Approved_material`、`cp_d`）內容可任意上傳／刪除——程式全面容錯；詳 `scripts/reference_intake/README.md`。
| 構面 | 內容 |
|---|---|
...
| 排程入口 | `scripts/reference_intake/daily_intake.py`（先清過期素材→跑 `stock_search.py`→Telegram 摘要；冪等：同日已入庫即跳過，`--force` 可重跑） |
| Mac 排程 | launchd `com.aidramafactory.reference.intake`——**每日 02:00**（plist 版控於 `scripts/reference_intake/`） |
