...
　v15.9：**新鮮度鐵律**——每部作品 dc=0 新歌 ≥ 50% · 廢除 v15.7 軟遞補 · `NewSongsInsufficientException` 硬閘門 · `--ttapi-fill-fresh` 自動補彈 · Tab1 新鮮度儀表卡 · 分頻道 `min_new_ratio`
...
| 14 | **🗄️ DB 路徑集中管理（v15.8）** | 所有 DB 存放於 `assets/data/`；引用必須用 `config.music_db_path` / `config.visual_db_path`，禁用 `workspace_root / "*.db"` |
| 15 | **🎛️ 編碼 Profile 動態讀取（v15.8）** | `common_enc` / `audio_enc` 必須從 `config.video_enc_profile` 讀取；禁止腳本中硬編碼 CRF/preset/bitrate |
| 16 | **🆕 新鮮度鐵律（v15.9）** | 每部 1 小時作品 dc=0 新歌 ≥ `ceil(target × min_new_ratio)`；不足即拋 `NewSongsInsufficientException` 中止；CEO 可用 `--ttapi-fill-fresh` 旗標自動補彈；UI Tab1 儀表卡即時顯示狀態並鎖定啟動按鈕 |
| 17 | **🎵 聽覺衍生唯一上限（3）** | `lofi_assembler --max-derivation-limit` 預設 **3**：僅 `derivation_count < 3`（Gen0/1/2）入混音池；**≥3** 冷凍退役（與 Protocol L `<3`、戰情表一致）。視覺片段為獨立參數。 |
...
### [Protocol Q: Phantom Matrix Assembly v15.8]（幻影矩陣廣播級拼接）

> **⚠️ `multi_scene_processor.py` 受 CEO 鎖定（v15.8）。見 Protocol LOCK。**

- **Trigger:** Protocol V 素材清單就緒，或 CEO 指定 `--bg-video / --bg-videos / --auto-visual`
- **Actor:** `QA`（Phantom Rotation Expert）
- **Action:** 執行 `multi_scene_processor.py` v15.8：
  ...
  **Stage 1（Encode-Once-Repeat + Pre-baked Fade + Pre-baked Reverse）— 四層廣播級防禦：**
  ...
  > - **v15.8 雙軌 Profile（由 `config.video_enc_profile` 動態讀取，禁止硬編碼）：**
  >   ```
  >   # Windows h264_win:   -c:v libx264 -preset slow -tune film -crf 29
  >   # Mac M4 hevc_mac:    -c:v hevc_videotoolbox -q:v 55 -tag:v hvc1
  >   # 兩者共同 GOP 防禦（鎖定）：
  >   -pix_fmt yuv420p
  >   -g 48 -keyint_min 48        # 嚴格 IDR：最大 = 最小 = 48 幀
  >   -sc_threshold 0             # 關閉動態場景偵測
  ...
  **Stage 2.5（Acrossfade Audio Premix）：**
  ...
  - 最終 AAC 輸出：`config.video_enc_profile["audio_enc"]` = `["-c:a", "aac", "-b:a", "192k", "-ar", "48000"]`（v15.8：從 320k 降至 **192k**，聽覺透明標準，消除 44100↔48000Hz 浮點換算漂移）
...
- **Constraint:**
  ...
  - **v15.8**：`common_enc` 必須從 `config.video_enc_profile["common_enc"]` 讀取，嚴禁腳本內硬編碼 CRF/preset
  - **v15.8**：Stage 2 音頻必須從 `config.video_enc_profile["audio_enc"]` 讀取（192k，嚴禁 320k）
...
### [Protocol VS: Vault Shortage Recovery]（庫存短缺恢復 v15.9 修訂）

- **Trigger:** Phase 4 被跳過（`庫存為 0`），或 `lofi_assembler.py` 拋出 `VaultShortageException`
- **Actor:** `AD`
- **RCA：** 僅在 `total_selected == 0`（真正空庫）時發生。若為 dc=0 新歌不足，改由 Protocol FP 處理（v15.9 新增）。
- **Action：**
  1. 確認 `vault_ready_for_mix/{channel}/` 實體 WAV 數量 → 若 0，觸發 `_run_suno_backup()` + `_sync_mastered_to_vault()`
  2. 若 DB 與磁碟不同步（DB 有記錄但檔案遺失），執行 `UPDATE audio_assets SET is_archived=1 WHERE ...` 歸檔幽靈記錄
  3. 重新執行產線
...
### [Protocol FP: Freshness Policy Enforcement]（新鮮度鐵律執行 v15.9 新增）

- **Trigger:** `lofi_assembler.NewSongsInsufficientException` 或 `pipeline_runner._check_freshness_quota()` 未通過
- **Actor:** `AD`（CTO 架構）+ `PM`（CEO 決策）
- **RCA：** v15.7 軟遞補允許 Gen1 掩蓋新歌荒，長期導致品牌發行衍生掛名「新」產品，損害訂閱信譽。v15.9 強制硬閘門。
- **Rule:** `selected_new ≥ ceil(target_tracks × min_new_ratio)`
  - `target_tracks = 20`（預設）
  - `min_new_ratio` 從 `config.freshness_policy.channels[channel].min_new_ratio` 讀取（lofi / light_music 均為 `0.5`）
  - 可用 `.env` 覆蓋：`FRESHNESS_RATIO_LOFI` / `FRESHNESS_RATIO_LIGHT_MUSIC`
- **Gates（兩道閘門，雙保險）：**
  1. **UI 閘門（app.py Tab1）：** 啟動按鈕前呼叫 `backend.can_start_pipeline()`，strict 模式未達標 → 按鈕鎖定 + 顯示「新鮮度 N / quota ❌」儀表卡，避免白跑 Phase 1-3
  2. **Pipeline 閘門（pipeline_runner `_check_freshness_quota()`）：** Phase 4 前置檢查，未達標 → `sys.exit(4)` 或啟用 `--ttapi-fill-fresh` 補彈
  3. **Assembler 閘門（lofi_assembler `VaultSelection.run()`）：** 選曲時 `selected_new < quota_new` → `raise NewSongsInsufficientException` → `sys.exit(4)`
- **Action（CEO 三選一）：**
  1. **手動上傳**（預設、免費）：將新 MP3 檔案放入 `assets/audio/ceo_approved_beats/{channel}/`，重新啟動產線
  2. **TTAPI 自動補彈**（有費用）：`pipeline_runner.py --ttapi-fill-fresh` 或 UI 開啟「🔫 啟用 TTAPI 自動補彈」開關。系統會呼叫 `_run_suno_backup(purpose="freshness")`，跳過 Protocol L（衍生無法補新），只調 Suno API 生成真正的 dc=0 曲目
  3. **warn 降級**（不建議）：`FRESHNESS_ENFORCEMENT=warn` → 僅警告不阻擋，但長期會破壞品牌新鮮度
- **Constraint:**
  - 不得從 Gen1 / Gen2 填補 New 配額缺口（v15.7 軟遞補已廢除）
  - Gen2 不足時只能從 Gen1 補，**絕不動用 New 池**
  - 任何腳本新增選曲邏輯時，`min_new_ratio` 必須從 `config.freshness_policy` 讀取，嚴禁硬編碼
- **DoD:** `selected_new ≥ quota_new`，`NewSongsInsufficientException` 未觸發，Phase 4 順利產出 WAV
- **Handoff:** 合規後進入 Protocol Q（Phantom Matrix Assembly）
...
| 檔案 | 內容 | 生成 Protocol |
|------|------|-------------|
| `R&S_Echoes_{ch}_1HrMix_[TS].mp4` | 1 小時廣播級無縫音畫同步影片（零靜止空洞 + 振盪場景零閃切）~1.57GB/hr（Win）| Protocol Q v15.8 |
