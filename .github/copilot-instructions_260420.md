...
# v15.10 — Mac 兩階段部署 · Streaming/OBC 實務運維 · 新鮮度硬閘門
# 最後更新: 2026-04-20（對齊 架構說明書 v15.10 與現行 Streaming 代碼）
...
#   v15.8: 雙軌編碼 Profile（h264_win CRF29/hevc_mac q:v 55）· DB 集中化（assets/data/）· 音頻 192k
#   v15.9: 新鮮度鐵律——dc=0 新歌 ≥ 50% · 廢除 v15.7 軟遞補 · NewSongsInsufficientException
#          + _check_freshness_quota() 前置閘門 · --ttapi-fill-fresh 旗標 · Tab1 儀表卡 · 分頻道 min_new_ratio
#   v15.10: Mac mini 兩階段部署（Phase 1 先營運、Phase 2 再遷移全自動）
#           + Streaming/ 正名（舊 Steam 廢止）+ 外接 SSD 全量部署 + README/manifest/rotate 對齊
#           + docs/AGENT_HANDOFF.md 與 docs/DUAL_CHANNEL_SOP_SSH_VNC.md 新增交接與雙通道 SOP
# 歸檔舊版: spec/copilot-instructions.v15.9.md
# ========================================================
#
# 你是一位頂級的結對編程專家 (Pair Programmer)。你必須嚴格遵守以下由 CTO 定義的
# v15.10「Mac 兩階段部署 · Streaming/OBC 實務運維 · 新鮮度鐵律 · CEO 鎖定核心引擎」產線鐵律。
# 你的任務是協助開發者實踐這些規則，絕不可給出違背此文件或舊版架構的建議。
# 參考文件: 架構說明書_v15.10.md、.clinerules、.openclaw/project.yml、
#          docs/AGENT_HANDOFF.md、docs/DUAL_CHANNEL_SOP_SSH_VNC.md、scripts/streaming_steam/README.md
...
1a. **Mac External-SSD Full Deployment（新增）:** On Mac mini, ALL production-critical artifacts (repo code, scripts, configs, Streaming/ MP4, manifests) MUST live under external-SSD WORKSPACE_ROOT at `/Volumes/AI_Workspace/AI_Drama_Factory`. Never split required runtime assets onto internal system disk.

1b. **Mac Phased Deployment（v15.10）:** Enforce two-phase strategy:
   - Phase 1: Mac mini is primarily 24/7 streaming operations node (OBS/RTMP and `Streaming/` runtime).
   - Phase 2: Promote to full automation host only after stable operations baseline is reached.
   Never mix migration-risk refactors into active streaming incident windows.
...
## 2. PIPELINE DISCIPLINE v15.9（產線核心鐵律）
...
5. **Broadcast-Grade Video Compression（v15.8 鎖定）:** Stage 1 鑄造素材的 `common_enc` 必須從 `config.video_enc_profile["common_enc"]` 動態讀取，嚴禁硬編碼 CRF/preset。Stage 2 音頻必須從 `config.video_enc_profile["audio_enc"]` 讀取（192k，非 320k）。嚴禁固定位元率 `-b:v`，嚴禁省略任何 Closed GOP 參數。
5a. **DB Path Rule（v15.8）:** 所有 DB 路徑必須用 `config.music_db_path` / `config.visual_db_path`（`assets/data/` 下）。嚴禁 `workspace_root / "*.db"` 根目錄 DB 引用。
...
8. **Streaming Naming and Root（v15.10）:** Live operations root folder is `Streaming/` (old `Steam/` is deprecated). Any guidance touching live broadcast assets must target `WORKSPACE_ROOT/Streaming` and remain consistent with `scripts/streaming_steam/README.md`.
...
## 7. BROADCAST-GRADE FFMPEG DISCIPLINE v15.8（廣播級 FFmpeg 鐵律）

> ⚠️ 以下所有參數由 CEO 鎖定於 `multi_scene_processor.py`。Copilot 不得建議移除或變更任何一項。

### Stage 1 編碼防禦陣列（v15.8 動態 Profile，完整 GOP 防禦不可拆分）

```python
# ✅ v15.8 鎖定版 — 從 config.video_enc_profile 動態讀取，禁止在此硬編碼任何 CRF/preset
...
# Stage 2 音頻（v15.8：從 320k → 192k）
audio_enc = config.video_enc_profile["audio_enc"]
# 展開：["-c:a", "aac", "-b:a", "192k", "-ar", "48000"]

# ❌ 禁止（v15.7 舊版硬編碼）
common_enc = ["-c:v", "libx264", "-preset", "medium", "-crf", "28", ...]  # 不可硬編碼
stage2_audio = ["-b:a", "320k"]  # v15.8 起嚴禁 320k
```
...
> **`scripts/gear1_prod/multi_scene_processor.py` 為 CEO 鎖定檔案（v15.6 起生效，v15.8 持續）。**
...
- **v15.8 新增：** 不得建議在 `multi_scene_processor.py` 中硬編碼 `common_enc` 或 `audio_enc` 參數
- **v15.8 新增：** 不得建議使用 `workspace_root / "*.db"` 路徑引用 DB，必須用 `config.music_db_path` / `config.visual_db_path`

**若用戶確認已獲 CEO 授權，修改後必須提醒：**
1. 先備份至 `spec/backups/v15.8_pre_[YYYYMMDD]/`
2. 執行 GOP-QA 沙盒驗證：
```powershell
...
## 10a. DB PATH CENTRALIZATION（DB 路徑集中管理規範 v15.8）

> **v15.8 起，所有 DB 存放於 `assets/data/`。`env_manager.py` 提供 `music_db_path` / `visual_db_path`。**

```python
# ✅ 正確：透過 config 讀取（v15.8 標準）
from scripts.common.env_manager import EnvConfig
config = EnvConfig()
music_db  = config.music_db_path    # → PosixPath("…/assets/data/rs_music_vault.db")
visual_db = config.visual_db_path   # → PosixPath("…/assets/data/veo_visual_vault.db")

# ❌ 禁止：根目錄引用（v15.7 舊版，assets/data/ 遷移後根目錄無 DB）
music_db  = config.workspace_root / "rs_music_vault.db"
visual_db = config.workspace_root / "veo_visual_vault.db"
```

**禁止事項：**
- 嚴禁在任何腳本中硬編碼 `F:/AI_DRAMA_FACTORY/rs_music_vault.db`
- 嚴禁不透過 `config` 直接寫 `Path("assets/data/rs_music_vault.db")`（相對路徑不跨平台）
- 更新 DB 相關腳本時必須使用 `config.music_db_path` 或 `config.visual_db_path`

---

## 10. AUDIO VAULT SELECTION（lofi_assembler 新鮮度鐵律 v15.9）

> `lofi_assembler.py` `VaultSelection.run()` v15.9 硬閘門邏輯。
> **v15.7 軟遞補（New 不足→Gen1 補）已廢除。**

```python
# ✅ v15.9 硬閘門
# 配額動態計算（從 config.freshness_policy 讀取 min_new_ratio，預設 0.5）
quota_new = math.ceil(self.target_tracks * self.min_new_ratio)
quota_gen1 = math.ceil((self.target_tracks - quota_new) * 0.5)
quota_gen2 = self.target_tracks - quota_new - quota_gen1

# Step 4a：新歌不足 → 硬閘門拋異常（不再從 Gen1 補）
...
if new_deficit > 0 and self.freshness_enabled and self.freshness_enforcement == "strict":
    raise NewSongsInsufficientException(
        f"新歌（dc=0）不足：需求 {quota_new}，實選 {len(selected_new)}，缺 {new_deficit}。"
        f"解法：上傳新 MP3 / --ttapi-fill-fresh / FRESHNESS_ENFORCEMENT=warn"
    )

# Step 4b：Gen2 不足 → 只從 Gen1 補（絕不動用 New 池）
...
# Step 5：total >= 1 繼續；total == 0 才 VaultShortageException
total_selected = len(selected_new) + len(selected_gen1) + len(selected_gen2)
if total_selected < 1:
    raise VaultShortageException("庫存完全為空")

# ❌ v15.7 廢除：New 不足從 Gen1 補
# if new_deficit > 0:
#     selected_gen1.extend(self._select_from_pool(pool_gen1, new_deficit))  # 禁止！

# ❌ v15.7 廢除：Gen2 不足從 New 補
# if gen2_deficit > 0:
#     selected_new.extend(self._select_from_pool(pool_new, gen2_deficit))  # 禁止！會破壞新鮮度
```

**pipeline_runner 前置閘門（v15.9）：**

```python
# ✅ Phase 4 之前必須呼叫
freshness = _check_freshness_quota(channel=channel, target_tracks=20)
if not freshness["passed"]:
    if ttapi_fill_fresh:
        _run_suno_backup(target_count=..., channel=channel, purpose="freshness")
        # 重新同步後複檢
    elif freshness["enforcement"] == "warn":
        pass  # 僅警告
    else:
        _log_fatal("FRESHNESS_VIOLATION", ...)
        sys.exit(4)
```
...
1. **音頻（歌曲）唯一上限: 3**（Gen0~Gen2，`derivation_count < 3` 為可用；≥3 冷凍退役）
   ```python
   # lofi_assembler.py argparse 預設
   parser.add_argument("--max-derivation-limit", type=int, default=3)
   
   # backend.py 函式簽名預設
   def run_phase4_sequence(self, max_audio_deriv: int = 3)
   ```
...
## 18. AGENT CONTINUITY & OPERATIONS RUNBOOK（v15.10 交接與維運）

1. **Fresh-session read order (mandatory):**
   - First read: `docs/AGENT_HANDOFF.md` and `scripts/streaming_steam/README.md`
   - Then task-scoped reads only (scripts/config actually being changed)
   - `架構說明書_v15.10.md` is reference-grade context; read selectively when cross-module decisions are needed

2. **Dual-channel operations model (SSH + VNC):**
   - SSH is the control plane (scripts, logs, process control, deterministic checks)
   - VNC is the visual verification plane (YouTube Studio state, GUI prompts, mount visibility)
   - Follow `docs/DUAL_CHANNEL_SOP_SSH_VNC.md` for pre-flight and incident triage

3. **Preferred Mac host for transfer/ops:** Use wired host `robert@192.168.1.102` by default unless human operator explicitly switches.

---

## 19. ZERO-WASTE WORKSPACE DISCIPLINE（零廢棄紀律）
...
## QUICK REFERENCE CARD v15.9（違禁對照速查）
...
max_derivation_limit 音頻預設 3（唯一標準）     視覺片段預設 2（獨立）
...
硬編碼 common_enc CRF/preset（v15.8）          config.video_enc_profile["common_enc"]（動態讀取）
Stage 2 音頻用 320k（v15.8）                   config.video_enc_profile["audio_enc"]（192k）
workspace_root / "rs_music_vault.db"（v15.8） config.music_db_path（→ assets/data/）
workspace_root / "veo_visual_vault.db"（v15.8）config.visual_db_path（→ assets/data/）
Phase 4 選曲 New 不足從 Gen1 補（v15.9）        raise NewSongsInsufficientException 中止
新鮮度配額硬編碼 0.5（v15.9）                   config.freshness_policy.channels[ch].min_new_ratio
pipeline_runner 跳過 _check_freshness_quota    Phase 4 前必須呼叫 + 未達標 sys.exit(4)
UI Tab1 啟動按鈕不檢查新鮮度                   backend.can_start_pipeline() 閘門，strict 鎖定按鈕
TTAPI 補彈時仍走 Protocol L 衍生                _run_suno_backup(purpose="freshness") 強制跳過 Protocol L
```
